...
CREATE TABLE PUBLIC_USER_INFORMATION AS (SELECT	* FROM UNNAMED.PUBLIC_USER_INFORMATION);
CREATE TABLE PUBLIC_ARE_FRIENDS AS (SELECT * FROM UNNAMED.PUBLIC_ARE_FRIENDS);
CREATE TABLE PUBLIC_PHOTO_INFORMATION AS (SELECT * FROM UNNAMED.PUBLIC_PHOTO_INFORMATION);
CREATE TABLE PUBLIC_TAG_INFORMATION AS (SELECT * FROM UNNAMED.PUBLIC_TAG_INFORMATION);
CREATE TABLE PUBLIC_EVENT_INFORMATION AS (SELECT * FROM UNNAMED.PUBLIC_EVENT_INFORMATION);
...
/*
  Home location is a one to one relation, so user_id is a foreign key here
*/
...
FOR EACH ROW
BEGIN
SELECT cur_sequence.nextval into :new.CUR_ID from dual; 
END; 
.
RUN;
...
CREATE TABLE ALBUM (
  ...
  CONSTRAINT vis_check CHECK (ALBUM_VISIBILITY = 'EVERYONE' OR
...
/*
  Photos to Album is a one-to-many relation, so Album_ID is a foreign key here
*/
CREATE TABLE PHOTO (
